...
def main(altitude, mach, wing_loading, plot=True):
    ...
    iteration_setup.weight_iter.TOW = 194_872.9
    iteration_setup.weight_iter.BOW = 103_146.1
    iteration_setup.weight_iter.FUEL = 67_225.9
    ...
    iteration_setup.sizing_iter.thrust_loading = 0.24
    ...
    aerodynamics.settings.drag_coefficient_increment.cruise = -12e-4 + 19e-4 -5e-4 - 10e-4
    ...
    if plot:
        ...
        txt = np.hstack((cl, cd_tot))
        np.savetxt('cl_cd.txt', txt.T, fmt='%.5f', delimiter=',')
        #plt.plot(cl_base, l_d_base, label='Baseline')
        #plt.plot(cl_ref, l_d_ref, label='Reference')
        plt.plot(cl, cl/cd_tot, label="SUAVE Recalibrated")
        plt.scatter(cl_calib, l_d_calib, label="FlightStream", color='r', marker='x')
        plt.xlabel(r'$C_L$')
        plt.ylabel(r'$L/D$')
        plt.grid('on')
