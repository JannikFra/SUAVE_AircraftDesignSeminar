...
def Baseline(parameters):
    ...
    if payload_range_run == True:
        plr = payload_range_multiple_cruise(configs.cruise, mission, num_cruise_seg=4, reserves=reserve_fuel_pct)
...
def sweep():
    ...
    speeds = np.array([0.84, 0.85])
    altitudes = np.arange(36000, 40000, 1000) * Units.ft
    X, Y = np.meshgrid(speeds, altitudes)
    fuels = np.zeros_like(X)
    for i, mach in enumerate(speeds):
