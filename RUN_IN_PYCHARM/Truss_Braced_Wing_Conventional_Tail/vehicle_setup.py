...
def vehicle_setup(iteration_setup):
    ...
    while abs(deltaaerocenterhtp) > 1e-10:
        ...
        wing.areas.reference = vehicle.wings.main_wing.spans.projected \
    ...
    vt_tip_xCoordinate = wing.origin[0][0] + wing.spans.projected * np.tan(wing.sweeps.quarter_chord) \
                       + wing.chords.root/4 - wing.chords.tip/4
    vt_tip_zCoordinate = wing.origin[0][2] + wing.spans.projected

    print('VT tip x: ', vt_tip_xCoordinate)
    print('VT tip z: ', vt_tip_zCoordinate)
    ...
    wing.origin = [[vt_tip_xCoordinate, 0, vt_tip_zCoordinate]]
    wing.aerodynamic_center = [1, 0, 0]
    ...
    while abs(deltaaerocenterhtp) > 1e-10:
        ...
        wing.areas.reference = vehicle.wings.main_wing.chords.mean_aerodynamic \
                             * vehicle.wings.main_wing.areas.reference \
                             * c_ht \
                             / l_ht

        # wing.spans.projected = (wing.aspect_ratio * wing.areas.reference) ** 0.5
        # wing.spans.total     = wing.spans.projected / np.cos(wing.dihedral)
        # wing.chords.root     = wing.areas.reference / wing.spans.projected * 2/(1 + wing.taper)
